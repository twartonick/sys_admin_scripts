...
##########################################################################
#                                                                        #
# Code modifications commented below copyright Thomas Wartonick 2015     #
# under same licence referenced below by original author.                #
#                                                                        #
##########################################################################
##########################################################################
#                                                                        #
...
###############################################################################
# Customization/modification begins here.
# 
# I wanted more info documented at time of load issue 
# for point in time root cause investigation:
# server-status, mysql, netstat, top
#
###############################################################################

{	
		elinks -dump  http://your_domain/server-status >> /var/log/cpanel_load_high/server_status_`date +%Y%m%d%H%M`.log &

        mysqladmin processlist >> /var/log/cpanel_load_high/mysql_processlist_`date +%Y%m%d%H%M`.log &

        netstat -anp >> /var/log/cpanel_load_high/netstat_output_`date +%Y%m%d%H%M`.log &
        wait

        cd /var/log/cpanel_load_high

        cat *`date +%Y%m%d%H%M`.log >> combined_log_`date +%Y%m%d%H%M`.log

        top -n2 -c -b >> /var/log/cpanel_load_high/combined_log_`date +%Y%m%d%H%M`.log
		
# Modification ends here

        SUBJECTLINE="`hostname` Problem see /var/log/cphost_load_high dir [L: $ONE_MIN_LOADAVG] [P: $MAX_PROCS_CHECK] [Swap Use: $TOTAL_PERCENTAGE ] [pps in: $PACKETS_RX pps out: $PACKETS_TX]"
		top -n2 -c -b  |  mail -s "$SUBJECTLINE" $EMAIL
